...
def concat_tplot_segments(base_name: str, segment_ids: List[Union[int, str]]):
    """
    セグメントに分割されたtplot変数を、時間軸に沿って一つに連結する。

    Args:
        base_name (str):
            連結したい変数のベース名 (例: 'tau_seg', 'beta_ion_seg')。
            末尾にセグメントIDが付くことを想定している。

        segment_ids (list):
            連結するセグメントIDのリスト (例: [0, 1, 2, 3])。

    Returns:
        xarray.DataArray or None:
            全てのセグメントが連結されたxarray.DataArray。
            該当するデータが一つも見つからなかった場合はNoneを返す。
    """
    
    # --- 1. 全セグメントのデータをリストに読み込む ---
    data_arrays_to_concat = []
    print(f"\n--- Loading segments for '{base_name}' ---")
    for sid in segment_ids:
        var_name = f"{base_name}_seg{sid}"
        try:
            data_quant = pt.data_quants[var_name]
            v_dim_size = len(data_quant.v) if 'v' in data_quant.coords else 'N/A'
            print(f"  - Loaded '{var_name}' with v_dim size: {v_dim_size}")
            data_arrays_to_concat.append(data_quant)
        except KeyError:
            print(f"  - 警告: tplot変数 '{var_name}' が見つかりません。スキップします。")
            continue
            
    if not data_arrays_to_concat:
        print(f"警告: ベース名 '{base_name}' に対応するデータが一つも見つかりませんでした。")
        return None
        
    if len(data_arrays_to_concat) == 1:
        print("  - セグメントが1つだけなので、連結処理をスキップします。")
        return data_arrays_to_concat[0]

    # --- 2. データの種類を判別 ---
    first_da = data_arrays_to_concat[0]
    dims = first_da.dims

    if 'v_dim' in dims and len(dims) > 1:
        # --- スペクトログラムデータの場合：NumPyで手動結合 ---
        print(f"\n--- Combining spectrograms for '{base_name}' (Manual NumPy method) ---")
        
        # 2a. マスター周波数軸を作成 (全セグメントの和集合)
        master_freq = np.array([], dtype=np.float64)
        for da in data_arrays_to_concat:
            master_freq = np.union1d(master_freq, da.v.values)
        print(f"  - Master frequency axis created with {len(master_freq)} points.")
        
        # 2b. 最終的な時間軸と、空のデータ配列を作成
        final_time = np.concatenate([da.time.values for da in data_arrays_to_concat])
        final_data = np.full((len(final_time), len(master_freq)), np.nan, dtype=np.float64)
        print(f"  - Final data array created with shape: {final_data.shape}")

        # 2c. 各セグメントのデータを、NumPyレベルで空の配列にコピー
        print("  - Filling master data array from segments...")
        current_pos = 0
        for da in data_arrays_to_concat:
            segment_freq = da.v.values
            _common_freqs, master_indices, segment_indices = np.intersect1d(
                master_freq, segment_freq, return_indices=True
            )
            
            n_time = len(da.time)
            target_slice = final_data[current_pos : current_pos + n_time, :]
            source_data_slice = da.values[:, segment_indices]
            target_slice[:, master_indices] = source_data_slice
            
            current_pos += n_time
            
        # 2d. 時間でソート
        sort_indices = np.argsort(final_time)
        final_time = final_time[sort_indices]
        final_data = final_data[sort_indices, :]
        print("  - Sorting final array by time.")

        # 2e. NumPy配列から、新しいクリーンなDataArrayを再構築
        result_da = xr.DataArray(
            data=final_data,
            dims=('time', 'v_dim'),
            coords={'time': final_time, 'v': ('v_dim', master_freq), 'spec_bins': ('v_dim', master_freq)}
        )
        print(f"--- Concatenation successful for '{base_name}' ---")
        return result_da

    else:
        # --- 通常の時系列データの場合 ---
        print(f"\n--- Concatenating time series for '{base_name}' ---")
        concatenated_data = xr.concat(data_arrays_to_concat, dim='time')
        print(f"--- Concatenation successful for '{base_name}' ---")
        return concatenated_data.sortby('time')
...
def load_and_prepare_data(cutoff_freq=1/6):
    ...
    try:
        ...
        data_dict['v_A_'] = concat_tplot_segments('Alfven_speed', seg_ids).interp(time=time_axis, method='linear') * 1E3
        data_dict['ion_cyclo_freq'] = concat_tplot_segments('ion_cyclo_freq', seg_ids).interp(time=time_axis, method='linear')
        data_dict['V_ion_fac_perp'] = concat_tplot_segments('v_sys_fac_perp_rolling', seg_ids).interp(time=time_axis, method='linear') * 1E3
        data_dict['V_th_proton'] = concat_tplot_segments('ion_thermal_speed', seg_ids).interp(time=time_axis, method='linear') * 1E3
        data_dict['C_s_proton'] = concat_tplot_segments('ion_acoustic_speed', seg_ids).interp(time=time_axis, method='linear') * 1E3
        data_dict['tau'] = concat_tplot_segments('tau', seg_ids).interp(time=time_axis, method='linear')
        data_dict['beta_i'] = concat_tplot_segments('beta_ion', seg_ids).interp(time=time_axis, method='linear')
